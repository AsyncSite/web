...
const ThreeSceneSubtle = lazy(() => import('../components/whoweare/ThreeSceneSubtle'));

// Team member data - 실제 팀원 데이터로 업데이트
export const whoweareSubtleTeamMembers = [
  {
    id: 'rene-choi',
    name: 'RENE CHOI',
    initials: 'RC',
...
export interface WhoWeAreSubtleMemberData {
  id: string;
  name: string;
  initials: string;
  role: string;
...
const WhoWeAreSubtlePage: React.FC = () => {
  const [whoweareSubtleSelectedMember, setWhoweareSubtleSelectedMember] = useState<WhoWeAreSubtleMemberData | null>(null);
  const [whoweareSubtleIsLoading, setWhoweareSubtleIsLoading] = useState(true);
  const [whoweareSubtleLoadError, setWhoweareSubtleLoadError] = useState<string | null>(null);
  const [whoweareSubtleShow3D, setWhoweareSubtleShow3D] = useState(true);
    ...
    const checkWebGLSupport = () => {
      try {
        ...
        if (!gl || isMobile || isLowPerformance) {
          setWhoweareSubtleShow3D(false);
          setWhoweareSubtleIsLoading(false);
        }
      } catch (e) {
        setWhoweareSubtleShow3D(false);
        setWhoweareSubtleIsLoading(false);
  ...
  const closeWhoWeareSubtleMemberPanel = () => {
    setWhoweareSubtleSelectedMember(null);
  };
  ...
  const FallbackUI = () => (
    <div className="whoweare-fallback-container">
      ...
      <div className="whoweare-fallback-grid">
        {whoweareSubtleTeamMembers.map((member) => (
          <div
            key={member.id}
            ...
            onClick={() => setWhoweareSubtleSelectedMember(member)}
          >
  ...
  return (
    <div className="whoweare-page-container">
      {whoweareSubtleShow3D ? (
        <>
          {/* Loading screen */}
          {whoweareSubtleIsLoading && (
          ...
          <Suspense fallback={null}>
            <ThreeSceneSubtle
              members={whoweareSubtleTeamMembers}
              onMemberSelect={setWhoweareSubtleSelectedMember}
              onLoadComplete={() => setWhoweareSubtleIsLoading(false)}
              onLoadError={(error) => {
                setWhoweareSubtleLoadError(error);
                setWhoweareSubtleShow3D(false);
                setWhoweareSubtleIsLoading(false);
              }}
            />
          ...
          <div className="whoweare-ui-overlay">
            <div className="whoweare-header">
              <h1>TEAM SPACE</h1>
              <div className="whoweare-subtitle">AsyncSite 크루들의 작업실 (Subtle)</div>
            </div>
      ...
      {whoweareSubtleSelectedMember && (
        <div className="whoweare-ui-overlay">
          <div 
            className="whoweare-member-panel active" 
            style={{ 
              '--member-color': whoweareSubtleSelectedMember.color, 
              '--member-color-dark': whoweareSubtleSelectedMember.darkColor 
            } as React.CSSProperties}
          >
            <button className="whoweare-close-btn" onClick={closeWhoWeareSubtleMemberPanel}>×</button>
            <div className="whoweare-member-avatar">{whoweareSubtleSelectedMember.initials}</div>
            <div className="whoweare-member-name">{whoweareSubtleSelectedMember.name}</div>
            <div className="whoweare-member-role">{whoweareSubtleSelectedMember.role}</div>
            <div className="whoweare-member-quote">{whoweareSubtleSelectedMember.quote}</div>
            <div className="whoweare-member-story">{whoweareSubtleSelectedMember.story}</div>
...
export default WhoWeAreSubtlePage;