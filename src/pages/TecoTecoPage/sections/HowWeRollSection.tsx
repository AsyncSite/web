  ...
  return (
    <section className="tecoteco-how-we-roll-section">
      ...
      <h2 className="section-title">
        특별한 건 없어요. <br /> <span className="highlight">그냥 계속</span> 모일 뿐이에요.
      </h2>
              ...
              {item.description && (
                <p>
                  {item.description.split(' ').map((word, i) =>
                    word === '오프라인' || word === '중심으로' ? (
                      <span key={i} className="color-primary-text">
                        {word}{' '}
                      </span>
                    ) : word === '백준,' || word === '프로그래머스를' ? (
                      <span key={i} className="color-secondary-text">
                        {word}{' '}
                      </span>
                    ) : (
                      word + ' '
      ...
      <p className="schedule-intro">{HOW_WE_ROLL_DATA.scheduleIntro}</p>
