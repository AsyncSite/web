...
interface ThreeSceneSubtleProps {
...
const ThreeSceneSubtle: React.FC<ThreeSceneSubtleProps> = ({ 
  members, 
  onMemberSelect, 
  onLoadComplete, 
  onLoadError 
  ...
  useEffect(() => {
    ...
    const initThree = async () => {
      try {
        ...
        // Renderer setup - 미세한 개선: toneMapping 추가
        ...
        for (let i = 0; i < particleCount * 3; i += 3) {
          ...
          // 미세한 색상 변화
          const color = new THREE.Color(0xC3E88D);
        ...
        members.forEach((member) => {
          ...
          // Sphere - 미세한 개선: MeshPhysicalMaterial 사용
          ...
          // Point light - 미세한 개선: 부드러운 개인 조명
          const light = new THREE.PointLight(new THREE.Color(member.color), 0.4, 6);
        ...
        const handleMouseMove = (event: MouseEvent) => {
          ...
          if (intersects.length > 0) {
            ...
            if (hoveredObject) {
              ...
              if (glow) {
                (glow as any).material.opacity = 0.25;
              }
            }
        ...
        const animate = () => {
          ...
          // Animate members - 미세한 개선: 더 부드러운 움직임
          memberObjects.forEach((obj) => {
            ...
            // 구체만 살짝 더 회전
            const sphere = obj.children.find((child: any) => child.geometry?.type === 'SphereGeometry');
            if (sphere) {
...
export default ThreeSceneSubtle;