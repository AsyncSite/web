...
import { hybridOnboardingStore } from './onboarding/OnboardingStore';
...
const ThreeSceneHybridOnboarding: React.FC<ThreeSceneFloatingStoryProps> = ({ 
  members, 
  onMemberSelect, 
  onLoadComplete, 
  onLoadError,
  ...
  const onDragInteractionRef = useRef(onDragInteraction);
  
  useEffect(() => {
    onMemberSelectRef.current = onMemberSelect;
    onStoryCardSelectRef.current = onStoryCardSelect;
    onDragInteractionRef.current = onDragInteraction;
  });
  ...
  useEffect(() => {
    ...
    const initThree = async () => {
      try {
        ...
        // Camera setup with entry animation
        ...
        if (!hasPlayedEntryAnimation.current) {
          // Start from far away for entry animation
          camera.position.set(0, 3, 50);
        } else {
          // Start at normal position if animation already played
          camera.position.set(0, 3, 25);
        }
        cameraRef.current = camera;
        
        // Entry animation - camera zoom in (only play once)
        if (!hasPlayedEntryAnimation.current) {
          hasPlayedEntryAnimation.current = true;
          setTimeout(() => {
            if (!mounted) return;
            const targetPos = { x: 0, y: 3, z: 25 };
            const startPos = { x: 0, y: 3, z: 50 };
            const duration = 3000;
        ...
        const createSmartHighlight = (target: any, intensity: number = 1) => {
          ...
          // Find if glow already exists
          const existingGlow = target.children.find((child: any) => child.name === 'glow');
          if (existingGlow) {
            // Animate existing glow
            const material = existingGlow.material;
            if (material && material.emissiveIntensity !== undefined) {
              ...
              const animateGlow = () => {
                const now = Date.now();
                const progress = Math.min((now - startTime) / duration, 1);
                material.emissiveIntensity = startIntensity + (targetIntensity - startIntensity) * progress;
          ...
          // Pulse animation
          const pulsate = () => {
            if (!mounted || !glowMesh.parent) return;
            const time = Date.now() * 0.001;
            const scale = 1.2 + Math.sin(time * 3) * 0.1;
            glowMesh.scale.set(scale, scale, scale);
            glowMaterial.opacity = 0.3 * intensity + Math.sin(time * 3) * 0.1;
        ...
        // Check for inactivity and apply smart highlights
        let inactivityTimer: any = null;
        const checkInactivity = () => {
          if (!hybridOnboardingStore.isActive()) return;
          
          const action = hybridOnboardingStore.checkInactivity();
          if (action === 'highlight-nearest' && storyObjects.length > 0) {
            // Find nearest story card and highlight it
            createSmartHighlight(storyObjects[0], 1.5);
          }
        };
        ...
        const resetInactivityTimer = () => {
          ...
          inactivityTimer = setTimeout(checkInactivity, 15000);
        };
        ...
        const handleMouseMove = (event: MouseEvent) => {
          ...
          if (mouseDownTimeRef.current > 0) {
            ...
            if (distance > 5) {
              ...
              // Record drag interaction for onboarding
              if (hybridOnboardingStore.isActive() && distance > 10) {
                hybridOnboardingStore.recordInteraction('drag');
                onDragInteractionRef.current?.();
                resetInactivityTimer();
              }
            }
          }
        ...
        const handleClick = (event: MouseEvent) => {
          ...
          if (clickedObject) {
            if (clickedObject.isMember) {
              ...
              // Record member interaction for onboarding
              if (hybridOnboardingStore.isActive()) {
                hybridOnboardingStore.recordInteraction('member', clickedObject.group.userData.id);
                resetInactivityTimer();
              }
            } else if (clickedObject.isStoryCard) {
              // Record story interaction for onboarding
              if (hybridOnboardingStore.isActive()) {
                hybridOnboardingStore.recordInteraction('story', clickedObject.group.userData.id);
                resetInactivityTimer();
              }
...
export default ThreeSceneHybridOnboarding;