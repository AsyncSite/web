...
interface ThreeSceneAIGuideProps {
...
const ThreeSceneAIGuide: React.FC<ThreeSceneAIGuideProps> = ({ 
  ...
  const aiGuideRef = useRef<any>(null);
  ...
  const onMemberSelectRef = useRef(onMemberSelect);
  const onStoryCardSelectRef = useRef(onStoryCardSelect);
  
  useEffect(() => {
    onMemberSelectRef.current = onMemberSelect;
    onStoryCardSelectRef.current = onStoryCardSelect;
  });
  ...
  useEffect(() => {
    ...
    const initThree = async () => {
      try {
        ...
        // AI Guide will be created in separate useEffect
        ...
        const handleClick = (event: MouseEvent) => {
          ...
          if (clickedObject) {
            if (clickedObject.isMember) {
              onMemberSelectRef.current(clickedObject.group.userData as WhoWeAreMemberData);
              ...
              setTimeout(() => {
                if (clickedObject.group.userData && onStoryCardSelectRef.current) {
                  onStoryCardSelectRef.current(clickedObject.group.userData);
                }
              }, stage3Start);
        ...
        const animate = () => {
          ...
          // Animate AI Guide (only if it exists in scene)
          if (aiGuideRef.current) {
            // Idle rotation
            aiGuideRef.current.rotation.y += 0.005;
            
            // Ring rotations
            if (aiGuideRef.current.children[1]) {
              aiGuideRef.current.children[1].rotation.z += 0.01;
            }
            if (aiGuideRef.current.children[2]) {
              aiGuideRef.current.children[2].rotation.x += 0.01;
            }
            if (aiGuideRef.current.children[3]) {
              aiGuideRef.current.children[3].rotation.y += 0.01;
            }
            
            // Particle rotation
            if (aiGuideRef.current.children[5]) {
              aiGuideRef.current.children[5].rotation.y += 0.002;
            }
            
            // Pulse effect for core
            if (aiGuideRef.current.children[4]) {
              const scale = 1 + Math.sin(time * 3) * 0.1;
              aiGuideRef.current.children[4].scale.setScalar(scale);
            }
            
            // Keep position fixed at 0,0,0 - no floating
          }
  ...
  useEffect(() => {
    if (!isThreeLoaded || !sceneRef.current) return;
    
    const createAIGuide = async () => {
      const THREE = await import('three');
      
      if (!showAIGuide) {
        // Fade out and remove if exists
        if (aiGuideRef.current) {
          ...
          const fadeOut = () => {
            ...
            aiGuide.scale.setScalar(scale);
            
            if (progress < 1) {
              requestAnimationFrame(fadeOut);
            } else {
              // Remove from scene
              sceneRef.current.remove(aiGuide);
              aiGuideRef.current = null;
            }
          };
      ...
      // Create AI Guide if doesn't exist
      if (!aiGuideRef.current) {
        const aiGuideGroup = new THREE.Group();
        
        // Main body (sphere with special material)
        const bodyGeometry = new THREE.IcosahedronGeometry(0.6, 2);
        ...
        bodyMaterial.userData.originalOpacity = 0.9;
        
        const body = new THREE.Mesh(bodyGeometry, bodyMaterial);
        aiGuideGroup.add(body);
        ...
        ringMaterial.userData.originalOpacity = 0.6;
        
        const ring1 = new THREE.Mesh(ringGeometry, ringMaterial);
        ring1.rotation.x = Math.PI / 2;
        aiGuideGroup.add(ring1);
        
        const ring2 = new THREE.Mesh(ringGeometry, ringMaterial.clone());
        ring2.userData.originalOpacity = 0.6;
        ring2.rotation.y = Math.PI / 2;
        aiGuideGroup.add(ring2);
        
        const ring3 = new THREE.Mesh(ringGeometry, ringMaterial.clone());
        ring3.userData.originalOpacity = 0.6;
        ring3.rotation.z = Math.PI / 3;
        aiGuideGroup.add(ring3);
        ...
        coreMaterial.userData.originalOpacity = 0.8;
        ...
        particlesMaterial.userData.originalOpacity = 0.8;
        
        const particles = new THREE.Points(particlesGeometry, particlesMaterial);
        aiGuideGroup.add(particles);
        
        // Position the AI Guide - fixed at center
        aiGuideGroup.position.set(0, 0, 0);
        
        // Start scaled down
        aiGuideGroup.scale.setScalar(0.7);
        
        // Store reference
        aiGuideRef.current = aiGuideGroup;
        
        // Add to scene
        sceneRef.current.add(aiGuideGroup);
        ...
        const fadeIn = () => {
          ...
          aiGuideGroup.traverse((child: any) => {
            if (child.material && child.material.userData.originalOpacity) {
              child.material.opacity = child.material.userData.originalOpacity * eased;
            }
          });
          
          // Scale up
          const scale = 0.7 + eased * 0.3;
          aiGuideGroup.scale.setScalar(scale);
    ...
    createAIGuide();
  }, [showAIGuide, isThreeLoaded]); // Remove aiGuidePosition dependency - always at 0,0,0

  return <div ref={mountRef} style={{ width: '100vw', height: '100vh' }} />;
};

export default ThreeSceneAIGuide;