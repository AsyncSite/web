...
export class GameManager {
  ...
  private async processTurn(guess: number[]): Promise<void> {
    ...
    const correctCount = guess.filter((idx) => this.gameContext.answers.includes(idx)).length;

    const isWinner =
      correctCount === this.config.answerCount &&
  ...
  private getHintRevealSchedule(playerInfo: PlayerInfo): {
    startTurn: number;
    ...
    if (playerInfo.type === 'built-in-ai') {
      ...
      console.log(
        `[${playerInfo.aiDifficulty} AI] 예상 게임 길이: ${expectedTurns}턴, 시작: ${startTurn}턴, 초기 간격: ${profile.initialInterval}턴`,
      );
  ...
  private updateAndGetRevealedHints(
    playerId: number,
    schedule: { startTurn: number; initialInterval: number; accelerationFactor: number },
  ): { playerId: number; hints: number[] }[] {
    ...
    if (schedule.accelerationFactor >= 1.5 && schedule.accelerationFactor < 8.0) {
      // Medium
      revealMultiplier = 0.3; // Medium: 기존 Easy 수준으로
    } else if (schedule.accelerationFactor >= 8.0) {
      // Hard
    ...
    if (shouldReveal) {
      ...
      if (schedule.accelerationFactor >= 3.0 && schedule.accelerationFactor < 8.0) {
        // Medium
        thresholdRatio = 0.4;
      } else if (schedule.accelerationFactor >= 8.0) {
        // Hard
      ...
      if (
        (revealedHints.length >= totalAvailableHints * thresholdRatio || (isHardAI && isEndgame)) &&
      ...
      const nextInterval = Math.max(
        ...
        Math.floor(
          schedule.initialInterval /
            (1 + (turnsPassedSinceStart + 1) / (10 / schedule.accelerationFactor)),
        ),
      );
    }
