...
function decideStrategy(gameState, knowledge, possibleSolutions) {
  ...
  if (knowledge.definiteAnswers.size === gameState.answerCount) {
    ...
    const hasCloseOpponent = Array.from(knowledge.opponentKnowledge.values()).some(
      (info) => info.closeToWin,
    );
...
function balancedApproach(gameState, knowledge, possibleSolutions) {
  ...
  // 높은 확률의 unknown 추가
  const candidates = Array.from(knowledge.probableAnswers.entries()).sort((a, b) => b[1] - a[1]);
  ...
  // 점수 순으로 선택
  const sorted = Array.from(knowledge.probableAnswers.entries()).sort((a, b) => b[1] - a[1]);
