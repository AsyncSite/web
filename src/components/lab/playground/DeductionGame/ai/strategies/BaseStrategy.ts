...
export abstract class BaseStrategy implements AIStrategy {
  ...
  private analyzeAllGuesses(
    ...
    gameState.previousGuesses.forEach((guess) => {
      if (guess.correctCount === 0) {
        ...
        console.log(
          `정답 0개 추측에서 오답 발견: ${guess.guess.map((i) => gameState.keywords[i])}`,
        );
      }
    });
  ...
  private preventDuplicateGuess(
    ...
    if (allPreviousGuesses.has(guessKey)) {
      ...
      if (previousGuess) {
        console.log(
          `[중복 발견] 이전 결과: ${previousGuess.correctCount}/${gameState.answerCount} (플레이어 ${previousGuess.playerId})`,
        );
        ...
        if (previousGuess.correctCount > 0 && previousGuess.correctCount < gameState.answerCount) {
          ...
          return this.generateSystematicVariation(
            guess,
            previousGuess,
            gameState,
            scores,
  ...
  private generateSystematicVariation(
    ...
    for (let replaceCount = 1; replaceCount <= toReplace; replaceCount++) {
      ...
      if (!allPreviousGuesses.has(newKey)) {
        console.log(
          `체계적 변형 (${replaceCount}개 교체): ${toReplaceIndices.map((i) => gameState.keywords[i]).join(', ')} → ${availableCandidates
            .slice(0, replaceCount)
            .map((i) => gameState.keywords[i])
            .join(', ')}`,
        );
        return newGuess;
      }
  ...
  private generateNewCombination(
    ...
    // 확실한 정답은 유지
    const mustInclude = originalGuess.filter((idx) => gameState.revealedAnswers.includes(idx));
    ...
    while (attempts < 10) {
      ...
      // 약간의 무작위성을 추가하여 선택
      const shuffledCandidates = [...candidates.slice(0, needed * 2)].sort(
        () => Math.random() - 0.5,
      );
