 ...
 */
export function findGuessesWithKeyword(
...
export function calculateExpectedInfoGain(
  ...
  for (let correct = 0; correct <= Math.min(proposedGuess.length, answerCount); correct++) {
    const prob = binomialProbability(proposedGuess.length, correct, guessProb / proposedGuess.length);
  ...
  outcomes.forEach((prob, correctCount) => {
    if (prob > 0) {
      ...
      const infoGain = Math.log2(proposedGuess.length) - Math.log2(Math.max(1, proposedGuess.length - correctCount));
...
export function analyzePlayerPattern(
  ...
  const variance = correctRates.reduce((sum, rate) => {
