...
export class ProbabilisticInference implements IInferenceEngine {
  ...
  private bayesianUpdate(guess: GuessHistory, gameState: GameStateForAI): void {
    ...
    for (let i = 0; i < gameState.keywords.length; i++) {
      ...
      if (inGuess.has(i)) {
        // 키워드가 추측에 포함된 경우
        likelihood = this.calculateLikelihoodInGuess(i, guessedKeywords, correctCount, gameState);
      } else {
        // 키워드가 추측에 포함되지 않은 경우
  ...
  private calculateLikelihoodNotInGuess(
    ...
    const likelihood = this.gaussianPDF(correctCount, expectedCorrect, Math.sqrt(variance));
  ...
  private computeConfidence(probabilities: Map<number, number>, gameState: GameStateForAI): number {
