...
export class StrategicPlanning {
  ...
  private static midGameStrategy(
    ...
    probabilities.forEach((prob, keyword) => {
      if (!gameState.revealedWrongAnswers.includes(keyword) &&
        ...
        const frequency = gameState.previousGuesses
          .filter(g => g.guess.includes(keyword)).length;
  ...
  private static calculateDynamicThreshold(gameState: GameStateForAI): number {
    const remainingTurns = gameState.maxTurns 
      ? gameState.maxTurns - gameState.currentTurn
      : 10; // 기본값
